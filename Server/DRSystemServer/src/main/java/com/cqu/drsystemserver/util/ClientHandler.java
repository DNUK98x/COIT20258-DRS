...
import com.cqu.drsystem.model.User;
import com.cqu.drsystemserver.service.*;
...
public class ClientHandler implements Runnable {
    ...
    private void handleRequest(String requestType, ObjectInputStream input, ObjectOutputStream output)
        ...
        switch (requestType) {
            ...
            case GET_DEPARTMENT_TYPE ->
                handleGetDepartmentType(input, output);
            case GET_ALL_USERS ->
                handleGetAllUsers(output);
            case UPDATE_USER ->
                handleUpdateUser(input, output);
            case DELETE_USER ->
                handleDeleteUser(input, output);
            case ALLOCATE_POLICE_RESOURCES ->
                handlePoliceAllocation(input, output);
            case ALLOCATE_HEALTH_RESOURCES ->
                handleHealthAllocation(input, output);
            case ALLOCATE_FIRE_RESOURCES ->
                handleFireAllocation(input, output);
            case GET_HEALTH_RESOURCES ->
                handleGetHealthResources(input, output);
            case GET_POLICE_RESOURCES ->
                handleGetPoliceResources(input, output);
            case GET_FIRE_RESOURCES ->
                handleGetFireResources(input, output);
            case UPDATE_HEALTH_RESOURCES ->
                handleUpdateHealthResources(input, output);
            case UPDATE_POLICE_RESOURCES ->
                handleUpdatePoliceResources(input, output);
            case UPDATE_FIRE_RESOURCES ->
                handleUpdateFireResources(input, output);
            case SAVE_DISASTER ->
                handleSaveDisaster(input, output);
            case GET_ALL_DISASTERS ->
                handleGetAllDisasters(output);
            case GET_USER_DISASTERS ->
                handleGetUserDisasters(input, output);
            case GET_DEP_DISASTERS ->
                handleGetDepartmentSpecificDisasters(input, output);
            default ->
                sendResponse(output, "Unknown request type");
        }
    }
    ...
    private void handleLogin(ObjectInputStream input, ObjectOutputStream output)
        ...
        User user = (User) userService.login(email, password);
    ...
    private void handleGetHealthResources(ObjectInputStream input, ObjectOutputStream output)
            throws IOException, ClassNotFoundException, SQLException {
        int disasterId = (int) input.readObject();
        HealthResource healthResources = (HealthResource) healthResourceService.getHealthResources(disasterId);
    ...
    private void handleGetPoliceResources(ObjectInputStream input, ObjectOutputStream output)
            throws IOException, ClassNotFoundException, SQLException {
        int disasterId = (int) input.readObject();
        PoliceResource policeResources = (PoliceResource) policeResourceService.getPoliceResources(disasterId);
    ...
    private void handleGetFireResources(ObjectInputStream input, ObjectOutputStream output)
            throws IOException, ClassNotFoundException, SQLException {
        int disasterId = (int) input.readObject();
        FireResource fireResources = (FireResource) fireResourceService.getFireResources(disasterId);
